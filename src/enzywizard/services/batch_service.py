...
def run_batch_service(
    ...
    logger.print(f"[INFO] Batch processing started: input_path={input_path}, input_msa={input_msa}, substrate_names={substrate_names}")


    if not file_exists(input_path):
        logger.print(f"[ERROR] Input protein file not found: {input_path}")
        return False
    ...
    protein_name = get_stem(input_path)
    ...
    batch_result = run_batch_workflow(
        ...
        add_H=add_H,
        pH=pH,
        clean_force_field_file=clean_force_field_file,
        cutoff_area=cutoff_area,
        minimize_energy=minimize_energy,
        minimization_iteration=minimization_iteration,
    ...
    if not save_extra_outputs:
        shutil.copy2(working_output_dir / log_filename, output_dir / log_filename)
